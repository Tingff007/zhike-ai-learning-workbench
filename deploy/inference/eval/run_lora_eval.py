# -*- coding: utf-8 -*-
"""LoRA 微调模型评估 — 回答质量评测脚本
向 WSL2 推理服务发送测试问题，记录回答和响应时间。
"""
import json
import os
import time
...
def call_lora(question, temperature=0.1, max_tokens=300):
    """调用 LoRA 推理服务"""
    payload = {
...
def main():
    ...
    for cat, results in sorted(categories.items()):
        ...
        print(f"  {cat}: {len(results)} 题, 平均耗时 {avg_lat:.1f}s")
    ...
    output_path = r"E:\testing\微调评估\lora_eval_results.json"
