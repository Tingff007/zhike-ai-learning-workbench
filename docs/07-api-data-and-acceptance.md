...
| AI 教学助手 | `POST /ta/agent/messages` | 教师端对话式 Agent：课程知识问答（本地 pgvector 混合检索 + 低置信度拒答 + 强制引用）与业务数据只读查询（班级/作业/提交/成绩/测验真实统计）。仅 `ta`/`admin` 可访问，回答证据不足时明确拒答（零幻觉防线）。 |
...
### 7.5 教师端 AI 教学助手验收（本地知识库零幻觉）

* 助教登录后可在 `/ta/ai-assistant` 打开 AI 教学助手对话页。
* 知识问答：选择已就绪课程后提问（如 data_system 问「什么是高可用系统」），返回带 `[1][2]` 编号引用的回答，引用面板展示切片来源与相似度。
* 零幻觉拒答：提问内容在知识库中无依据或相似度低于 `RAG_RETRIEVAL_MIN_SCORE`（当前 0.55）时，Agent 明确拒答并说明原因（`no_hit`/`low_confidence`），不编造事实。
* 业务数据查询：提问「我名下的班级和作业情况怎么样」等，返回数据库真实统计（班级数、学生数、作业/提交/批改/测验数）与可核验事实卡片，不调用大模型、不产生虚构数据。
* 会话轨迹：前端展示安全审查 → 意图路由 → 课程解析 → 本地知识库检索 → 回答生成 → 引用核验六步轨迹。
* 教师端 Agent 仅执行只读查询与检索，不触发发布/删除/评分等副作用操作。
